<template>
    <main class="form-signin w-100 m-auto">
        ...
        <div v-if="user" class="d-flex flex-column gap-3"
            ...
            <div>
                <label class="form-label fw-semibold">마케팅 동의:</label>
                <div class="border rounded p-2 text-muted">{{ user.maAgree ? '동의' : '미동의' }}</div>
            </div>
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div class="d-flex gap-2 mt-4 flex-wrap">
                ...
                <button @click="deleteUser" class="btn btn-danger btn-small flex-fill">
                    삭제
                </button>
            </div>
            <div class="d-flex gap-2 mt-4 flex-wrap  justify-content-center">
                <button type="button" class="btn btn-success btn-small flex-fill" @click="$router.push('/User-List')">
                    목록
                </button>
            </div>
        </div>
